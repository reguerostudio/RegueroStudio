...
export default function CardVideo({ title, tag, src, thumb, aspectRatio = 'landscape' }: CardVideoProps) {
  const aspectClass =
  ...
  return (
    <div className="group relative mb-4 overflow-hidden" style={{ borderRadius: '4px' }}>
      ...
      <div className={`relative w-full ${aspectClass} overflow-hidden`}>
        {thumb ? (
          <img src={thumb} alt={title} className="h-full w-full object-cover transition-transform duration-700 group-hover:scale-105" />
        ) : (
          /* placeholder */
          <div
